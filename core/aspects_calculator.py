...
class AspectsCalculator:
    ...
    def _calculate_aspects_manual(
        ...
        if active_points:
            active_lower = [p.lower() for p in active_points]
            planet_names = [p for p in planet_names if p in active_lower or p.capitalize() in active_points]
    ...
    def calculate_dual_chart_aspects(
        ...
        try:
            ...
            try:
                ...
                if aspects:
                    logger.info(f"Найдено {len(aspects)} межкарточных аспектов через AspectsFactory")
                    return aspects
                    
            except (ImportError, AttributeError, Exception) as e:
                logger.info(f"AspectsFactory.dual_chart_aspects не сработал: {e}, переходим к ручному расчету")
            
            # Ручной расчет межкарточных аспектов
            return cls._calculate_dual_aspects_manual(subject1, subject2, active_points, include_minor)
    ...
    @classmethod
    def _calculate_dual_aspects_manual(
        cls,
        subject1,
        subject2,
        active_points: Optional[List[str]] = None,
        ...
        """
        Ручной расчет аспектов между двумя картами.
        """
        aspects_list = []
        
        # Список планет для обоих субъектов
        planet_names = [
            "sun", "moon", "mercury", "venus", "mars",
            "jupiter", "saturn", "uranus", "neptune", "pluto"
        ]
        ...
        # Собираем позиции планет для первого субъекта
        planets1 = []
        for name in planet_names:
            planet = getattr(subject1, name, None)
            if planet is not None:
                abs_pos = getattr(planet, 'abs_pos', None)
                if abs_pos is None:
                    sign_num = getattr(planet, 'sign_num', 0)
        ...
        # Собираем позиции планет для второго субъекта
        planets2 = []
        for name in planet_names:
            planet = getattr(subject2, name, None)
            if planet is not None:
                abs_pos = getattr(planet, 'abs_pos', None)
                if abs_pos is None:
                    sign_num = getattr(planet, 'sign_num', 0)
        ...
        # Перебираем все пары планет между картами
        for p1 in planets1:
            for p2 in planets2:
                # Вычисляем угол
                angle = abs(p1['abs_pos'] - p2['abs_pos'])
        ...
        logger.info(f"Ручной расчет межкарточных аспектов: найдено {len(aspects_list)} аспектов")
        return aspects_list